...
#include "../projects/ESPRESSIF/esp32/main/azure_iot_freertos_esp32_main.h"
                                               ...
                                               AzureIoTJSONWriter_t *pxWriter)
{
    ...
    if ((xResult = AzureIoTJSONReader_NextToken(pxReader)) != eAzureIoTSuccess)
    {
        LogError(("Error getting next token: result 0x%08x", xResult));
                                                          ...
                                                          &ulSinceTimeLength)) != eAzureIoTSuccess)
    {
        LogError(("Error getting token string: result 0x%08x", xResult));
    }
    else if ((xResult = AzureIoTJSONWriter_AppendBeginObject(pxWriter)) != eAzureIoTSuccess)
    {
        LogError(("Error appending begin object: result 0x%08x", xResult));
                                                                         ...
                                                                         xDeviceMaximumTemperature, sampleazureiotDOUBLE_DECIMAL_PLACE_DIGITS)) != eAzureIoTSuccess)
    {
        LogError(("Error appending max temp: result 0x%08x", xResult));
                                                                         ...
                                                                         xDeviceMinimumTemperature, sampleazureiotDOUBLE_DECIMAL_PLACE_DIGITS)) != eAzureIoTSuccess)
    {
        LogError(("Error appending min temp: result 0x%08x", xResult));
                                                                         ...
                                                                         xDeviceAverageTemperature, sampleazureiotDOUBLE_DECIMAL_PLACE_DIGITS)) != eAzureIoTSuccess)
    {
        LogError(("Error appending average temp: result 0x%08x", xResult));
                                                                         ...
                                                                         ucCommandStartTimeValueBuffer, ulSinceTimeLength)) != eAzureIoTSuccess)
    {
    ...
    /* Faking the end time to simplify dependencies on <time.h> */
                                                                         ...
                                                                         sizeof(sampleazureiotCOMMAND_FAKE_END_TIME) - 1)) != eAzureIoTSuccess)
    {
        LogError(("Error appending end time: result 0x%08x", xResult));
    }
    else if ((xResult = AzureIoTJSONWriter_AppendEndObject(pxWriter)) != eAzureIoTSuccess)
    {
                           ...
                           uint32_t *ulTelemetryDataLength)
{
    ...
    int result = snprintf((char *)pucTelemetryData, ulTelemetryDataSize,
                          motorcontroliotMESSAGE, timestamp, direction, duty_cycle, velocity, position, current);
