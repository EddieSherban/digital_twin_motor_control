...
void CurrentSensor::adc_task(void *arg)
{
  ...
  while (1)
  {
    temp_voltage = curr_sen_obj->read_voltage();
    curr_sen_obj->voltage = voltage_average.next(temp_voltage);
    temp_current = (curr_sen_obj->voltage - curr_sen_obj->zero_voltage) / MV_TO_MA;
    curr_sen_obj->current = current_average.next(temp_current);
    // ESP_LOGI(TAG, "%d", curr_sen_obj->voltage);
...
void CurrentSensor::zero()
{
  ...
  {
    temp_voltage = read_voltage();
    temp_zero_voltage = zero_average.next(temp_voltage);
  }
