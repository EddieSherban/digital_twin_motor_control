...
static constexpr int8_t TX_TASK_CORE = 1;                             // Run task on Core 1
...
static constexpr int8_t RX_TASK_CORE = 1;                             // Run task on Core 1
...
void Communication::init()
{
  ...
  ESP_ERROR_CHECK(uart_driver_install(UART_NUM_0, RX_BUFFER_SIZE, 0, 0, NULL, 0));
  ESP_ERROR_CHECK(uart_param_config(UART_NUM_0, &uart_config));
  ESP_ERROR_CHECK(uart_set_pin(UART_NUM_0, GPIO_TX, GPIO_RX, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
